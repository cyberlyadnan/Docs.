...
//         tabIndex="-1"
//         aria-hidden="true"
//         className="fixed top-0 right-0 left-0 z-50 flex justify-center items-center w-full h-full bg-black bg-opacity-70"
//       >
...
const Modal = ({ onClose, onSubmit, note }) => {
  const [title, setTitle] = useState("");
  const [content, setContent] = useState("");
  ...
  const handleSubmit = (e) => {
    ...
    if (onSubmit) {
      onSubmit({
        ...
        title,
        content,
        date: note ? note.date : new Date().toLocaleDateString(),
        time: note ? note.time : new Date().toLocaleTimeString(),
      });
      setTitle("");
      setContent("");
      onClose();
